...
class NativeContextMenuPlugin : public flutter::Plugin {
 ...
 private:
  ...
  void NativeContextMenuPlugin::ShowMenu(
...
void NativeContextMenuPlugin::RegisterWithRegistrar(
  ...
  auto channel =
      std::make_unique<flutter::MethodChannel<flutter::EncodableValue>>(
          registrar->messenger(), "native_context_menu",
...
void NativeContextMenuPlugin::CreateMenu(HMENU menu,
  ...
  for (flutter::EncodableValue item_value : items) {
    ...
    std::string title =
        std::get<std::string>(item_map.at(flutter::EncodableValue("title")));

    UINT_PTR item_id = id;
    UINT uFlags = MF_STRING;

    flutter::EncodableList sub_items = std::get<flutter::EncodableList>(
        item_map.at(flutter::EncodableValue("items")));
...
void NativeContextMenuPlugin::ShowMenu(
  ...
  double device_pixel_patio =
      std::get<double>(args.at(flutter::EncodableValue("devicePixelRatio")));
  const flutter::EncodableList pos = std::get<flutter::EncodableList>(
      args.at(flutter::EncodableValue("position")));
  ...
  int x = static_cast<int>((std::get<double>(pos[0]) * device_pixel_patio) +
                           rect.left);
  int y = static_cast<int>((std::get<double>(pos[1]) * device_pixel_patio) +
                           rect.top + titleBarHeight);

  last_show_menu_result = std::move(result);
  last_show_menu_responded = false;

  TrackPopupMenu(hMenu, TPM_LEFTALIGN | TPM_LEFTBUTTON, static_cast<int>(x),
